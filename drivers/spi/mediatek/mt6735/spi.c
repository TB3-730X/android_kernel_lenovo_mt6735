...
#ifdef CONFIG_OF
struct pinctrl *pinctrl_spi;
struct pinctrl_state *pins_spi_mode0;
struct pinctrl_state *pins_spi_mode1;
...
static int spi_set_gpio_info(struct platform_device *pdev , int spi_pin_mode)
{
	...
	pins_spi_mode0 = pinctrl_lookup_state(pinctrl_spi, "spi-mode0");
	if (IS_ERR(pins_spi_mode0)) {
		ret = PTR_ERR(pins_spi_mode0);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl pins_spi_mode0!\n");
		return ret;
	}

	pins_spi_mode1 = pinctrl_lookup_state(pinctrl_spi, "spi-mode1");
	if (IS_ERR(pins_spi_mode1)) {
		ret = PTR_ERR(pins_spi_mode1);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl pins_spi_mode1!\n");
		return ret;
	}

	if (spi_pin_mode == 1) {
		pinctrl_select_state(pinctrl_spi, pins_spi_mode1);
	} else {
		pinctrl_select_state(pinctrl_spi, pins_spi_mode0);
	}

	dev_info(&pdev->dev, "fwq spi_set_gpio_info is success!\n");
...
static int mt_spi_setup(struct spi_device *spidev)
{
	...
	SPI_INFO(&spidev->dev, "tx_endian:%d, rx_endian:%d, rx_mlsb:%d, tx_mlsb:%d, cpol:%d, cpha:%d\n", 
		chip_config->tx_endian, chip_config->rx_endian, chip_config->rx_mlsb, chip_config->tx_mlsb, chip_config->cpol ,chip_config->cpha);
