...
//Other 20160121, guanyuwei.wt disable workaround as ES ALPS02519336
#if 0
...
#endif
//Other 20160121, guanyuwei.wt end
...
kal_bool upmu_is_chr_det(void)
{
	...
	/*else {*/
		...
		if (mt_usb_is_device()) {
			...
			pr_err("[upmu_is_chr_det] ACA Cable plug in\n");
...
//+Other_platform_req huangfusheng.wt ADD 20150522 start charging/stop charging
...
//Other huangfusheng.wt 20140826 add high temp charging process
...
#ifndef WT_VBAT_LOW_PERCENT_TRACKING
...
static kal_bool mt_battery_0Percent_tracking_check(void)
{
	...
	battery_log(BAT_LOG_CRTI, "0Percent, VBAT=%d system off=%d UI_SOC=%d\r\n",
...
#ifdef WT_HIGH_TEMP_CHARGING_SUPPORT
			if(((BMT_status.temperature > 45)&&(BMT_status.temperature <= 50))&&(BMT_status.bat_full == KAL_TRUE)) {
...
//Other guanyuwei@wt 151202 use I_bat instead of I_charger
#if defined(SOC_BY_HW_FG)
	//actually, this ICharging is the battery current
...
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if (BMT_status.bat_vol > 0) {
			mt_battery_update_status();
			//guanyuwei@wt, 160308, do not skip battery update in normal boot
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case FGADC_CURRENT_READ :
		user_data_addr = (int *)arg;
		ret = copy_from_user(adc_in_data, user_data_addr, 8);
		adc_out_data[0] = battery_meter_get_charging_current();//BMT_status.ICharging;
		ret = copy_to_user(user_data_addr, adc_out_data, 8);
		break;
		/* add for meta tool------------------------------- */
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
	...
	unsigned int hv_voltage = BATTERY_VOLT_10_500000_V;
...
static void battery_timer_resume(void)
{
	...
	//Other 20160121, guanyuwei.wt add for syncing temperature in resume func
	for (i = 0; i < 5; i++) {
		temperature += battery_meter_get_battery_temperature();
		msleep(1);
	}
	...
	battery_xlog_printk(BAT_LOG_CRTI, "Sync temperature in resume, now temperature = %d\n", BMT_status.temperature);
	//Other 20160121 add end
