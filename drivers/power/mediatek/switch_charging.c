...
//+Other_user_req  huangfusheng.wt	20150514 add set charging current in different temperature
			...
			if((BMT_status.temperature > batt_cust_data.min_charge_temperature)&&(BMT_status.temperature <= 15)) {
				g_temp_CC_value = AC_CHARGER_CURRENT_LOW_TEMP;
			} else if((BMT_status.temperature > 45)&&(BMT_status.temperature <= batt_cust_data.max_charge_temperature)){
				g_temp_CC_value = AC_CHARGER_CURRENT_HIGH_TEMP;
			} else {
				g_temp_CC_value = batt_cust_data.ac_charger_current;
			}
#else
...
//Other_platform_modify  huangfusheng.wt ADD 20150514  ui_soc sync full in charging
	...
	battery_log(BAT_LOG_CRTI, "[BATTERY] charging_full_check status = %d\n", status);
	if (status == KAL_TRUE) {
		g_full_check_count++;
...
#if !defined(CONFIG_MTK_JEITA_STANDARD_SUPPORT)
		if(batt_cust_data.high_battery_voltage_support) {
			//Other_user_req  huangfusheng.wt  20150514 add set different cv in different temperature
			#if defined(WT_BAT_CHARGE_CV_TEMP_LIMIT)
			if((BMT_status.temperature > batt_cust_data.min_charge_temperature)&&(BMT_status.temperature <= 45)) {
