...
void NL_receive_data(struct sk_buff *__skb)
{
	...
	else
	{
		...
		case PACKFILTER_IOCTL_GET_IPSTATRSU:
			{				
				...
				if (g_nPid)
				{
					...
					if (nValue <= 0)
					{
						printk(KERN_ERR "netlink_unicast failed on GetNodeStatResult! Return value is %ld\n", nValue);
					}
				}
				else
				{
		...
		case PACKFILTER_IOCTL_SET_CLSBYUID:
			...
			break;
		default:
			printk(KERN_ERR "PackFilter: Unknown command type (0x%lX)\n", nValue);
			break;
		}
	}
	kfree_skb(skb);
...
#ifdef __MY_ANDROID__
					else if (*pUid < 10000)	
#elif defined __ANDROID3__
					else if ((*pUid < 10000) && (*pUid > 0))
#elif defined __ANDROID_T4__	
					else if ((*pUid < 10000) && (*pUid != 1000))
#else	
					else if (*pUid < 1000)	
#endif	
					{
