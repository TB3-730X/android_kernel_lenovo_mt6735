...
#include "s5k3m2mipi_Sensor_sunny.h"
#include "s5k3m2_pdafotp_sunny.h"

#define PFX "S5K3M2SUNNY_camera_sensor"

#define LOG_INF(format, args...) pr_debug(PFX " [%s] " format, __func__, ##args)

#define LOG_1 LOG_INF("S5K3M2SUNNY,MIPI 4LANE\n")
...
#define S5K3M2SUNNY_OTP_FUNCTION
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
	...
	LOG_INF("framerate = %d, min framelength should enable is %d \n", framerate,min_framelength_en);
...
static void pip_capture_setting(void)
{
	...
	// e bruce, 2014/12/01
	LOG_INF( "S5K3M2SUNNY PIP setting Enter!"); 
	write_cmos_sensor(0x6028,0x4000);
	write_cmos_sensor(0x32CA,0x022B);
	write_cmos_sensor(0x32D6,0x022B);
	write_cmos_sensor(0x0344,0x0010);//bpc off, no matter mirrir flip, PV & cap FOV the same
...
*
*************************************************************************/
extern kal_uint8  ModuleHouseID_sunny ;
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12)+1;
...
#ifdef  S5K3M2SUNNY_OTP_FUNCTION
	LOG_INF("S5K3M2SUNNY OTP start \n");
	S5K3M2SUNNY_Otp_Info();
	if(ModuleHouseID_sunny != 0x0){
	LOG_INF("S5K3M2SUNNY  ModuleHouseID_sunny %d",ModuleHouseID_sunny);
              *sensor_id = 0xFFFFFFFF;
               return ERROR_SENSOR_CONNECT_FAIL;
	}
	//S5K3M2SUNNY_Otp_LSC_Read();
	//S5K3M2SUNNY_Otp_AF_Read();
	//S5K3M2SUNNY_Otp_AWB_Read();
	LOG_INF("S5K3M2SUNNY OTP end \n");
#endif
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	else //PIP capture: 24fps for less than 13M, 20fps for 16M,15fps for 20M
    {
		if (imgsensor.current_fps != imgsensor_info.cap1.max_framerate)
			LOG_INF("Warning: current_fps %d fps is not support, so use cap1's setting: %d fps!\n",imgsensor.current_fps/10,imgsensor_info.cap1.max_framerate/10);   
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:	
			...
			read_3m2_eeprom_sunny((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;			
...
UINT32 S5K3M2SUNNY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	...
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	S5K3M2SUNNY_MIPI_RAW_SensorInit	*/
