...
//BYTE s5k3m2v2_eeprom_data[DATA_SIZE]= {0};
...
#endif
bool read_3m2_eeprom_v2( kal_uint16 addr, BYTE* data, kal_uint32 size){
	//int i;
	addr = 0xb9c;
	size = 1404;
	...
	//if(!_read_s5k3m2_eeprom(addr, s5k3m2v2_eeprom_data, size)){
		//return false;
	//}
...
kal_uint8  ModuleHouseIDV2 = 0;

bool S5K3M2V2_Otp_Info(void )
{
	...
	info_flag=s5k3m2_otp_read(0x0400);
      ...
      if(0x01==info_flag){
		...
		if(otp_info.check_info==checksum){
			ModuleHouseIDV2=otp_info.Module_ID;
...
#define  S5K3M2V2_LSC_SIZE  (0x0b86-0x043b+1)
#define  S5K3M2V2_LSC_Base_Addr 0x43b
kal_uint8  s5k3m2v2_lsc[S5K3M2V2_LSC_SIZE] ={0};

bool S5K3M2V2_Otp_LSC_Read(void)
{
	kal_uint16 lsc_flag=0,i=0,index=0;
	kal_uint32 checksum=0,check_value=0;;
		
	lsc_flag=s5k3m2_otp_read(0x043a);
	index=S5K3M2V2_LSC_Base_Addr;
	if(0x01==lsc_flag){
		for(i=0;i<S5K3M2V2_LSC_SIZE;i++){
			s5k3m2v2_lsc[i]=s5k3m2_otp_read(index);
		      //LOG_INF("S5K3M2: index=0x%x value=0x%x \n",index,s5k3m2_lsc[i]);
			checksum += s5k3m2v2_lsc[i];		
			index++;
		}
...
kal_uint8 s5k3m2v2_awb[16]={0};

bool S5K3M2V2_Otp_AWB_Read(void)
{
	...
	if(0x01==wb_flag){
		...
		if(wbcheck==checksum){
			...
			s5k3m2v2_awb[0]=wb_unit_r_h;
			s5k3m2v2_awb[1]= wb_unit_r_l;
			s5k3m2v2_awb[2]=wb_unit_b_h;
			s5k3m2v2_awb[3]=wb_unit_b_l;
			s5k3m2v2_awb[4]=wb_unit_gr_h;
			s5k3m2v2_awb[5]=wb_unit_gr_l;
			s5k3m2v2_awb[6]=wb_unit_gb_h;
			s5k3m2v2_awb[7]=wb_unit_gb_l;
			s5k3m2v2_awb[8]=wb_golden_r_h;
			s5k3m2v2_awb[9]=wb_golden_r_l;
			s5k3m2v2_awb[10]=wb_golden_b_h;
			s5k3m2v2_awb[11]=wb_golden_b_l;
			s5k3m2v2_awb[12]=wb_golden_gr_h;
			s5k3m2v2_awb[13]=wb_golden_gr_l;
			s5k3m2v2_awb[14]=wb_golden_gb_h;
			s5k3m2v2_awb[15]=wb_golden_gb_l;
		}else{
