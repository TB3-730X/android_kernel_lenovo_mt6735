...
#include "ov5693mipi_Sensor_qtech.h"


#define PFX "OV5693_camera_sensor_qtech"
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)



#define LOG_1 LOG_INF("OV5693_qtech,MIPI 2LANE\n")
#define LOG_2 LOG_INF("OV5693_qtech preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV5693_SENSOR_ID_QTECH,        //record sensor id defined in Kd_imgsensor.h
...
#define RG_Ratio_Typical 0x130
#define BG_Ratio_Typical 0x119
...
static int check_otp_wb(int index)
{
		...
		if (flag == 0x00) 
		{
			LOG_INF("%s:group index is empty \n",__func__);
			return 0;
		}
		else if (flag & 0x80)
		{
			LOG_INF("%s:group index has invalid dat  \n",__func__);
			return 1;
		}
		else 
		{
			LOG_INF("%s:group index has valid data  \n",__func__);
			return 2;
		}
}
// index: index of otp group. (1, 2, 3)
...
static int read_otp_wb(int index, struct otp_struct *otp_ptr)
{
		...
		LOG_INF("%s:module id:%d,len id:%d,year:%d,month:%d,day:%d \n",__func__,(*otp_ptr).module_integrator_id,(*otp_ptr).lens_id,(*otp_ptr).production_year,(*otp_ptr).production_month,(*otp_ptr).production_day);
...
static int read_otp_lenc(int index, struct otp_struct *otp_ptr)
{
		...
		for(i=0;i<15;i++) 
		{
			(* otp_ptr).lenc[i]=read_cmos_sensor(address);
			address++;
		}
		// clear otp buffer
		for (i=0;i<16;i++) 
...
static int update_awb_gain(int R_gain, int G_gain, int B_gain)
{
		...
		LOG_INF("%s:R_gain %d,G_gain:%d,B_gain %d \n",__func__,R_gain,G_gain,B_gain);
		return 0;
}
...
static int update_otp_wb(void)
{
		...
		if (i>3) {
		  ...
		  LOG_INF("%s:no valid wb OTP data \n",__func__);
			return 1;
		}
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
			LOG_INF("quanzhen2 sensor id: 0x%x\n", *sensor_id);
