/*
   liuying@wind-mobi.com 20150404 begin
*/
/*****************************************************************************
 *
 * Filename:
...
#include "hi545qtechmipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "HI545QTECH"
#define LOG_1 LOG_INF("HI545,MIPI 2LANE\n")
#define LOG_2 LOG_INF("preview 1296*972@30fps; video 1296*972@30fps; capture 5M@30fps\n")
/****************************   Modify end    *******************************************/

//#define LOG_INF(format, args...)    xlog_printk(ANDROID_LOG_INFOPFX, "[%s] " format, __FUNCTION__, ##args)
#define LOG_INF(format, args...)    printk(PFX "[%s] " format, __FUNCTION__, ##args)


//#define HI545Qtech_DEBUG

#ifdef HI545Qtech_DEBUG
	#define LOG_DB(format, args...)    printk(PFX "[%s] " format, __FUNCTION__, ##args)
#else
    #define LOG_DB(format, args...)
#endif
...
char Hynix_i2c_buf1[HYNIX_SIZEOF_I2C_BUF];
static kal_uint16 hynix_i2c_burst_mode(struct Hynix_Sensor_reg table[]);
// Hynix burst mode End

static imgsensor_info_struct imgsensor_info = {
    .sensor_id = HI545QTECHMIPI_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
	...
	.pre = {
		...
		.mipi_data_lp2hs_settle_dc  = 20, //zhengzhou.wt modify for camera runin fail 2016.3
    ...
    .sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb,//sensor output first pixel color   //liuying@wind-mobi.com 20150302 
...
static imgsensor_struct imgsensor = {
    .mirror = IMAGE_H_MIRROR,                //mirrorflip information    //liuying@wind-mobi.com 20150302 
...
/////////////////////////////////////////////////////////////////////////////////////////
//liuying@wind-mobi.com 20150302 add begin
#define HI545QTECH_USE_AWB_OTP
#ifdef HI545QTECH_USE_AWB_OTP
...
#define RG_Ratio_Typical 0x164
#define BG_Ratio_Typical 0x137

 struct HI545QTECH_otp_struct 
{
	kal_uint16 R_Gain;
	kal_uint16 B_Gain;
	kal_uint8   awb_status;
};
static  struct HI545QTECH_otp_struct current_otp = {0};
#if 0
static void Hi545_Otp_Initial_Setting(void)
{
...
#endif
#if 1  //+++ zhengzhou.wt 2016.03.28 camera open fail
static void Hi545_OTP_Read_Write_End(void)
{
    ...
    LOG_INF("Hi545  OTP Read Write finished!!!\n");

}
#endif
//--- zhengzhou.wt 2016.03.28 camera open fail
static void Hi545OTPSetting(void)
{	
       Hi545_bytewrite_cmos_sensor(0x0A02, 0x01);//Fast sleep on
	Hi545_bytewrite_cmos_sensor(0x0118, 0x00); //sleep On
	mdelay(100);
...
static kal_uint16 Hi545_Sensor_OTP_read(kal_uint16 otp_addr,kal_uint16 val1,kal_uint16 val)
{
    kal_uint16 data;	
    ...
    Hi545_bytewrite_cmos_sensor(0x0102, 0x00); //single read
    	mdelay(10);
...
static void Hi545_Sensor_OTP_info(void)
{
	uint16_t Calibration_Ver =0,
           AFflag = 0,
			 Year = 0,  
			 Month = 0, 
			 Day = 0, 
    ...
    LOG_DB("HI545_Sensor info_flag = 0x%x \n", info_flag);
    switch (info_flag)
	{	
		case 0x40:
		ModuleHouseID = Hi545_Sensor_OTP_read(0x1802,0,0);
             Calibration_Ver = Hi545_Sensor_OTP_read(0x1803,0,0);
             AFflag = Hi545_Sensor_OTP_read(0x180d,0,0);
             ...
             Color_ID   = Hi545_Sensor_OTP_read(0x180c,0,0);
             Light_flag = Hi545_Sensor_OTP_read(0x180e,0,0);
             Cam_position = Hi545_Sensor_OTP_read(0x180f,0,0);
             infocheck = Hi545_Sensor_OTP_read(0x1811,0,0);
    	break;
		case 0xd0:
		ModuleHouseID = Hi545_Sensor_OTP_read(0x1812,0,0);
             Calibration_Ver = Hi545_Sensor_OTP_read(0x1813,0,0);
             AFflag = Hi545_Sensor_OTP_read(0x181d,0,0);
             Year = Hi545_Sensor_OTP_read(0x1814,0,0);
             Month = Hi545_Sensor_OTP_read(0x1815,0,0);
             Day = Hi545_Sensor_OTP_read(0x1816,0,0);
             SensorID = Hi545_Sensor_OTP_read(0x1817,0,0);
             LensID = Hi545_Sensor_OTP_read(0x1818,0,0);
             VCMID = Hi545_Sensor_OTP_read(0x1819,0,0);
             DriverID = Hi545_Sensor_OTP_read(0x181a,0,0);
             FnumberID = Hi545_Sensor_OTP_read(0x181b,0,0);
             Color_ID   = Hi545_Sensor_OTP_read(0x181c,0,0);
             Light_flag = Hi545_Sensor_OTP_read(0x181e,0,0);
             Cam_position = Hi545_Sensor_OTP_read(0x181f,0,0);
             infocheck = Hi545_Sensor_OTP_read(0x1821,0,0);
		break;
		case 0xf4:
		ModuleHouseID = Hi545_Sensor_OTP_read(0x1822,0,0);
            Calibration_Ver = Hi545_Sensor_OTP_read(0x1823,0,0);
            AFflag = Hi545_Sensor_OTP_read(0x182d,0,0);
            Year = Hi545_Sensor_OTP_read(0x1824,0,0);
            Month = Hi545_Sensor_OTP_read(0x1825,0,0);
            Day = Hi545_Sensor_OTP_read(0x1826,0,0);
            SensorID = Hi545_Sensor_OTP_read(0x1827,0,0);
            LensID = Hi545_Sensor_OTP_read(0x1828,0,0);
            VCMID = Hi545_Sensor_OTP_read(0x1829,0,0);
            DriverID = Hi545_Sensor_OTP_read(0x182a,0,0);
            FnumberID = Hi545_Sensor_OTP_read(0x182b,0,0);
            Color_ID   = Hi545_Sensor_OTP_read(0x182c,0,0);
            Light_flag = Hi545_Sensor_OTP_read(0x182e,0,0);
            Cam_position = Hi545_Sensor_OTP_read(0x182f,0,0);
            infocheck = Hi545_Sensor_OTP_read(0x1831,0,0);
		break;
		default:		
		LOG_INF("HI545_Sensor: info_flag error value: %d \n ",info_flag);
		break;
	...
	checksum = (ModuleHouseID +Calibration_Ver+ AFflag + Year + Month + Day + SensorID + LensID + VCMID + DriverID + FnumberID+Color_ID+Light_flag+Cam_position) % 0xFF + 1;
    ...
    LOG_DB("ModuleHouseID = %x \n", ModuleHouseID);
    LOG_DB("Year = %d, Month = %d, Day = %d\n", Year, Month, Day);    
    LOG_DB("LensID = %x \n", LensID);
    LOG_DB("VCMID = %x \n", VCMID);
    LOG_DB("DriverID = %x \n", DriverID);
...
static void Hi545_Sensor_update_wb_gain(struct HI545QTECH_otp_struct *otp)
{
    kal_uint16 rg,bg;
...
static void  Hi545_Sensor_calc_wbdata(struct HI545QTECH_otp_struct *otp)
{
    uint16_t wbcheck = 0, 
			 checksum = 0, 
			 wb_flag = 0;
	...
	wb_flag = Hi545_Sensor_OTP_read(0x1832,0,0);
	...
	{
		case 0x40:
		wb_unit_rg_h = Hi545_Sensor_OTP_read(0x1833,0,0);
		wb_unit_rg_l = Hi545_Sensor_OTP_read(0x1834,0,0);
		wb_unit_bg_h = Hi545_Sensor_OTP_read(0x1835,0,0);
		wb_unit_bg_l = Hi545_Sensor_OTP_read(0x1836,0,0);
		wb_unit_gg_h = Hi545_Sensor_OTP_read(0x1837,0,0);
		wb_unit_gg_l = Hi545_Sensor_OTP_read(0x1838,0,0);
		wb_golden_rg_h = Hi545_Sensor_OTP_read(0x1839,0,0);
		wb_golden_rg_l = Hi545_Sensor_OTP_read(0x183a,0,0);
		wb_golden_bg_h = Hi545_Sensor_OTP_read(0x183b,0,0);
		wb_golden_bg_l = Hi545_Sensor_OTP_read(0x183c,0,0);
		wb_golden_gg_h = Hi545_Sensor_OTP_read(0x183d,0,0);
		wb_golden_gg_l = Hi545_Sensor_OTP_read(0x183e,0,0);
		wb_unit_r_h = Hi545_Sensor_OTP_read(0x183f,0,0);
		wb_unit_r_l = Hi545_Sensor_OTP_read(0x1840,0,0);
		wb_unit_b_h = Hi545_Sensor_OTP_read(0x1841,0,0);
		wb_unit_b_l = Hi545_Sensor_OTP_read(0x1842,0,0);
		wb_unit_gr_h = Hi545_Sensor_OTP_read(0x1843,0,0);
		wb_unit_gr_l = Hi545_Sensor_OTP_read(0x1844,0,0);
		wb_unit_gb_h = Hi545_Sensor_OTP_read(0x1845,0,0);
		wb_unit_gb_l = Hi545_Sensor_OTP_read(0x1846,0,0);
		wb_golden_r_h = Hi545_Sensor_OTP_read(0x1847,0,0);
		wb_golden_r_l = Hi545_Sensor_OTP_read(0x1848,0,0);
		wb_golden_b_h = Hi545_Sensor_OTP_read(0x1849,0,0);
		wb_golden_b_l = Hi545_Sensor_OTP_read(0x184a,0,0);
		wb_golden_gr_h = Hi545_Sensor_OTP_read(0x184b,0,0);
		wb_golden_gr_l = Hi545_Sensor_OTP_read(0x184c,0,0);
		wb_golden_gb_h = Hi545_Sensor_OTP_read(0x184d,0,0);
		wb_golden_gb_l = Hi545_Sensor_OTP_read(0x184e,0,0);
		wbcheck = Hi545_Sensor_OTP_read(0x184f,0,0);
    	break;
		case 0xd0:
		wb_unit_rg_h = Hi545_Sensor_OTP_read(0x1850,0,0);
		wb_unit_rg_l = Hi545_Sensor_OTP_read(0x1851,0,0);
		wb_unit_bg_h = Hi545_Sensor_OTP_read(0x1852,0,0);
		wb_unit_bg_l = Hi545_Sensor_OTP_read(0x1853,0,0);
		wb_unit_gg_h = Hi545_Sensor_OTP_read(0x1854,0,0);
		wb_unit_gg_l = Hi545_Sensor_OTP_read(0x1855,0,0);
		wb_golden_rg_h = Hi545_Sensor_OTP_read(0x1856,0,0);
		wb_golden_rg_l = Hi545_Sensor_OTP_read(0x1857,0,0);
		wb_golden_bg_h = Hi545_Sensor_OTP_read(0x1858,0,0);
		wb_golden_bg_l = Hi545_Sensor_OTP_read(0x1859,0,0);
		wb_golden_gg_h = Hi545_Sensor_OTP_read(0x185a,0,0);
		wb_golden_gg_l = Hi545_Sensor_OTP_read(0x185b,0,0);
		wb_unit_r_h = Hi545_Sensor_OTP_read(0x185c,0,0);
		wb_unit_r_l = Hi545_Sensor_OTP_read(0x185d,0,0);
		wb_unit_b_h = Hi545_Sensor_OTP_read(0x185e,0,0);
		wb_unit_b_l = Hi545_Sensor_OTP_read(0x185f,0,0);
		wb_unit_gr_h = Hi545_Sensor_OTP_read(0x1860,0,0);
		wb_unit_gr_l = Hi545_Sensor_OTP_read(0x1861,0,0);
		wb_unit_gb_h = Hi545_Sensor_OTP_read(0x1862,0,0);
		wb_unit_gb_l = Hi545_Sensor_OTP_read(0x1863,0,0);
		wb_golden_r_h = Hi545_Sensor_OTP_read(0x1864,0,0);
		wb_golden_r_l = Hi545_Sensor_OTP_read(0x1865,0,0);
		wb_golden_b_h = Hi545_Sensor_OTP_read(0x1866,0,0);
		wb_golden_b_l = Hi545_Sensor_OTP_read(0x1867,0,0);
		wb_golden_gr_h = Hi545_Sensor_OTP_read(0x1868,0,0);
		wb_golden_gr_l = Hi545_Sensor_OTP_read(0x1869,0,0);
		wb_golden_gb_h = Hi545_Sensor_OTP_read(0x186a,0,0);
		wb_golden_gb_l = Hi545_Sensor_OTP_read(0x186b,0,0);
		wbcheck = Hi545_Sensor_OTP_read(0x186c,0,0);
		break;
		case 0xf4:
		wb_unit_rg_h = Hi545_Sensor_OTP_read(0x186d,0,0);
		wb_unit_rg_l = Hi545_Sensor_OTP_read(0x186e,0,0);
		wb_unit_bg_h = Hi545_Sensor_OTP_read(0x186f,0,0);
		wb_unit_bg_l = Hi545_Sensor_OTP_read(0x1870,0,0);
		wb_unit_gg_h = Hi545_Sensor_OTP_read(0x1871,0,0);
		wb_unit_gg_l = Hi545_Sensor_OTP_read(0x1872,0,0);
		wb_golden_rg_h = Hi545_Sensor_OTP_read(0x1873,0,0);
		wb_golden_rg_l = Hi545_Sensor_OTP_read(0x1874,0,0);
		wb_golden_bg_h = Hi545_Sensor_OTP_read(0x1875,0,0);
		wb_golden_bg_l = Hi545_Sensor_OTP_read(0x1876,0,0);
		wb_golden_gg_h = Hi545_Sensor_OTP_read(0x1877,0,0);
		wb_golden_gg_l = Hi545_Sensor_OTP_read(0x1878,0,0);
		wb_unit_r_h = Hi545_Sensor_OTP_read(0x1879,0,0);
		wb_unit_r_l = Hi545_Sensor_OTP_read(0x187a,0,0);
		wb_unit_b_h = Hi545_Sensor_OTP_read(0x187b,0,0);
		wb_unit_b_l = Hi545_Sensor_OTP_read(0x187c,0,0);
		wb_unit_gr_h = Hi545_Sensor_OTP_read(0x187d,0,0);
		wb_unit_gr_l = Hi545_Sensor_OTP_read(0x187e,0,0);
		wb_unit_gb_h = Hi545_Sensor_OTP_read(0x187f,0,0);
		wb_unit_gb_l = Hi545_Sensor_OTP_read(0x1880,0,0);
		wb_golden_r_h = Hi545_Sensor_OTP_read(0x1881,0,0);
		wb_golden_r_l = Hi545_Sensor_OTP_read(0x1882,0,0);
		wb_golden_b_h = Hi545_Sensor_OTP_read(0x1883,0,0);
		wb_golden_b_l = Hi545_Sensor_OTP_read(0x1884,0,0);
		wb_golden_gr_h = Hi545_Sensor_OTP_read(0x1885,0,0);
		wb_golden_gr_l = Hi545_Sensor_OTP_read(0x1886,0,0);
		wb_golden_gb_h = Hi545_Sensor_OTP_read(0x1887,0,0);
		wb_golden_gb_l = Hi545_Sensor_OTP_read(0x1888,0,0);
		wbcheck = Hi545_Sensor_OTP_read(0x1889,0,0);
		break;
		default:
	...
	rg_value = (((wb_unit_rg_h << 2) | ((wb_unit_rg_l>>6) & 0x0003)) & 0x03ff);                          //((wb_unit_rg_h<<8)&0xff00) |(wb_unit_rg_l &0xff);   
	bg_value = (((wb_unit_bg_h << 2) | ((wb_unit_bg_l>>6) & 0x0003)) & 0x03ff);            //((wb_unit_bg_h<<8)&0xff00) |(wb_unit_bg_l &0xff); 			           

	LOG_DB(" Read from module: wb_unit_rg_h = 0x%x,wb_unit_rg_l = 0x%x wb_unit_bg_h = 0x%x,wb_unit_bg_l = 0x%x wb_unit_gg_h = 0x%x,wb_unit_gg_l = 0x%x\n",
   ...
   // LOG_INF("Read from module: rg_value = 0x%x,bg_value = 0x%x\n",rg_value ,bg_value);
    ...
    LOG_DB(" After calcuate: rg_golden_value = 0x%x, wb_golden_rg_h = 0x%x,wb_golden_rg_l = 0x%x\n",rg_golden_value,wb_golden_rg_h,wb_golden_rg_l);
    LOG_DB(" After calcuate: g_gain = 0x%x, r_gain = 0x%x,b_gain = 0x%x rg_value = 0x%x \n",g_gain,r_gain,b_gain,rg_value);
	otp->R_Gain = r_gain;
	otp->B_Gain = b_gain;
...
////////////////////////////////////////////////////////////////////////////////////
//liuying@wind-mobi.com 20150302 add end
...
{
   // return read_cmos_sensor(0x0F17);
...
static kal_uint16 hynix_i2c_burst_mode(struct Hynix_Sensor_reg table[])
{
	...
	for (curr = table; curr->addr != HYNIX_TABLE_END; curr++) 
    {
		if (!buf_count) { 
			...
			Hynix_i2c_buf1[buf_count] = curr->addr >> 8; buf_count++;
            
			Hynix_i2c_buf1[buf_count] = curr->addr & 0xFF; buf_count++;
		}
        
		Hynix_i2c_buf1[buf_count] = curr->val >> 8; buf_count++;
        
		Hynix_i2c_buf1[buf_count] = curr->val & 0xFF; buf_count++;
        ...
        err = iWriteRegI2C(Hynix_i2c_buf1, buf_count, imgsensor.i2c_write_id);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0B1A, 0x1065); //1044);
	...
	write_cmos_sensor(0x0C0E, 0x2500); //0x07 BLC display On, 0x05 BLC D off //BLC_ctl3. Frame BLC On = 0x05, off=0x04 //FBLC_ctl3. [0]en_fblc, [1]en_blc_bypass, [2]en_fblc_dpc, [5]en_fobp_dig_offset, [7]en_lobp_dpc_bypass 
	write_cmos_sensor(0x0C10, 0x0510); //dig_blc_offset_h
	write_cmos_sensor(0x0C16, 0x4040); //fobp_dig_b_offset. red b[7] sign(0:+,1:-), b[6:0] dc offset 128(max)
	write_cmos_sensor(0x0C18, 0x4040); //fobp_dig_Gb_offset. Gr b[7] sign(0:+,1:-), b[6:0] dc offset 128(max)
	write_cmos_sensor(0x0C36, 0x0100); //r_g_sum_ctl. [0]:g_sum_en. '1'enable, '0'disable.

	//--- MIPI blank time --------------//
	write_cmos_sensor(0x0902, 0x4301); //mipi_value_clk_trail. MIPI CLK mode [1]'1'cont(0x43),'0'non-cont(0x41) [6]'1'2lane(0x41), '0'1lane(0x01) 
	...
	write_cmos_sensor(0x0018, 0x0ACD); //x_addr_end_hact_h. 
	...
	write_cmos_sensor(0x0026, 0x0016); //y_addr_start_vact_h. 
	write_cmos_sensor(0x002C, 0x07B1); //y_addr_end_vact_h.  
...
{ 
	...
	write_cmos_sensor(0x0B1A, 0x1065); //1044); //140718
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF(" HI545 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                break;
            }
            else{
                LOG_INF(" HI545  Read sensor id fail, write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
          }
            retry--;
        } while(retry > 0);
        i++;
    ...
    if (*sensor_id != imgsensor_info.sensor_id) {
      ...
      sensor_init();
	  //liuying@wind-mobi.com 20150302 add begin
	  Hi545OTPSetting();//+++ zhengzhou.wt 2016.03.28 camera open fail
	  Hi545_Sensor_OTP_info();
	  Hi545_Sensor_calc_wbdata(&current_otp);
	  Hi545_OTP_Read_Write_End();//---- zhengzhou.wt 2016.03.28 camera open fail
	  //liuying@wind-mobi.com 20150302 add end

     if(ModuleHouseID != 0x06)
		{
        *sensor_id = 0xFFFFFFFF;
        return ERROR_SENSOR_CONNECT_FAIL;
		}
...
/*
   liuying@wind-mobi.com 20150404 end
*/
