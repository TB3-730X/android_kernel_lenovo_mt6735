...
#define VOL_1000 1000000
	...
	if (On) {
		...
		if (pinSetIdx == 0 && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_HI545_MIPI_RAW))) {
		    ...
		    else if(pinSetIdx == 1 && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_SP2508_MIPI_RAW)))
		    {
		       PK_DBG("SP2508 power enter \n");
				...
				mtkcam_gpio_set(pinSetIdx, CAMRST, pinSet[pinSetIdx][IDX_PS_CMRST + IDX_PS_ON]);
				mdelay(10);
				printk("SP2508 power end \n");

			}
			else if (pinSetIdx == 0 && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_HI545QTECH_MIPI_RAW))) {
			/* First Power Pin low and Reset Pin Low */
			if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMPDN])
				mtkcam_gpio_set(pinSetIdx, CAMPDN, pinSet[pinSetIdx][IDX_PS_CMPDN + IDX_PS_OFF]);
			...
			if (pinSetIdx == 0 && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_HI545_MIPI_RAW))) {
				...
				PK_DBG("[CAMERA SENSOR] Fail to OFF digital power (VCAM_A), power id = %d \n", VCAMA);
				//goto _kdCISModulePowerOn_exit_;	
				}
				/* VCAM_IO */
