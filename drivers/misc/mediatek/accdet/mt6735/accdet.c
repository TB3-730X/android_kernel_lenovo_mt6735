...
struct pinctrl_state *pins_hs_out0;
struct pinctrl_state *pins_hs_out1;
#endif
#ifdef DEBUG_THREAD
#endif
...
#ifndef CONFIG_ACCDET_SGM3799
		pinctrl_select_state(accdet_pinctrl1, pins_hs_out0);
#endif
...
#ifndef CONFIG_ACCDET_SGM3799
		mdelay(300);
		pinctrl_select_state(accdet_pinctrl1, pins_hs_out1);
		mdelay(150);
#endif
...
static inline int accdet_setup_eint(struct platform_device *accdet_device)
{
	...
	pins_hs_out0 = pinctrl_lookup_state(accdet_pinctrl1, "hs_out0");
	if (IS_ERR(pins_hs_out0)) {
		ret = PTR_ERR(pins_hs_out0);
		dev_err(&accdet_device->dev, "fwq Cannot find accdet pinctrl state_eint_accdet!\n");
		return ret;
	}
    	pins_hs_out1 = pinctrl_lookup_state(accdet_pinctrl1, "hs_out1");
	if (IS_ERR(pins_hs_out1)) {
		ret = PTR_ERR(pins_hs_out1);
		dev_err(&accdet_device->dev, "fwq Cannot find accdet pinctrl state_eint_accdet!\n");
		return ret;
	}
       pinctrl_select_state(accdet_pinctrl1, pins_hs_out0);
