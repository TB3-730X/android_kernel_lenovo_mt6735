...
//Other guanyuwei@wt,160225,add for decribing secure info
#include <linux/hardware_info.h>
extern char ret_hw_secure_info[64];
extern char ret_sw_secure_info[64];

/* define /sys/bootinfo/hwsec_info */
static ssize_t hwsec_info_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	LOGE("hwsec=%s\n", ret_hw_secure_info);
	return sprintf(buf, "%s\n", ret_hw_secure_info);
}
...
/* define /sys/bootinfo/swsec_info */
static ssize_t swsec_info_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	LOGE("swsec=%s\n", ret_sw_secure_info);
	return sprintf(buf, "%s\n", ret_sw_secure_info);
}

static struct kobj_attribute swsec_info_attr = __ATTR_RO(swsec_info);
//add end

static struct attribute *bootinfo_attrs[] = {
	&powerup_reason_attr.attr,
	&hwsec_info_attr.attr,
	&swsec_info_attr.attr,
	NULL
};
