...
struct pinctrl *pinctrlled;
 ...
 struct led_gpio_attr {
	const char *name;
	bool gpio_prepare;
	struct pinctrl_state *gpioctrl;
};

static struct led_gpio_attr led_gpios[GPIO_NUM] = {
	[GPIO_RED_OUT0] = {"red-gpio-out0", false, NULL},
	[GPIO_RED_OUT1] = {"red-gpio-out1", false, NULL},
};

int mt_led_set_gpio(unsigned int level)
{
	int retval = 0;
	pr_info("mt_led_set_gpio:%d Start\n", level);
	if (level == 0) {
		if (led_gpios[GPIO_RED_OUT0].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlled, led_gpios[GPIO_RED_OUT1].gpioctrl);
			if (retval)
				pr_err("could not set led_gpios[GPIO_RED_OUT0] pins\n");
		}
	} else {
		if (led_gpios[GPIO_RED_OUT1].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlled, led_gpios[GPIO_RED_OUT0].gpioctrl);
			if (retval)
				pr_err("could not set led_gpios[GPIO_RED_OUT1] pins\n");
		}

	}
	pr_info("mt_led_set_gpio End\n");
	return retval;
}
#endif
...
#ifdef CONFIG_OF
	pinctrlled = devm_pinctrl_get(&client->dev);
	if (IS_ERR(pinctrlled)) {
		err = PTR_ERR(pinctrlled);
		pr_err("Cannot find pinctrlaud!\n");
		return err;
	}
	for (i = 0; i < ARRAY_SIZE(led_gpios); i++) {
		led_gpios[i].gpioctrl = pinctrl_lookup_state(pinctrlled, led_gpios[i].name);
		if (IS_ERR(led_gpios[i].gpioctrl)) {
			err = PTR_ERR(led_gpios[i].gpioctrl);
			pr_err("%s pinctrl_lookup_state %s fail %d\n", __func__, led_gpios[i].name, err);
		} else {
			led_gpios[i].gpio_prepare = true;
			pr_err("%s pinctrl_lookup_state %s success\n", __func__, led_gpios[i].name);
		}
	}
#endif
