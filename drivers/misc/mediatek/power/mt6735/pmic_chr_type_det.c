...
#define low_voltage_threshold 150//mv
#define high_voltage_threshold 700//mv
...
int hw_otg_get_aca_type_detection(void)
{
	...
	int times=1, IDDIG_CHANNEL_NUM = 0; //(AUXIN2)
	...
	while (i--)
	{
		...
		pr_err("[%s]: AUXADC Channel %d, ADC_voltage %d\n", __func__, IDDIG_CHANNEL_NUM, ADC_voltage);
	}

	if ( (high_voltage_threshold > ADC_voltage) && (ADC_voltage > low_voltage_threshold))
		{
		return ACA_C;
		}
	else 
