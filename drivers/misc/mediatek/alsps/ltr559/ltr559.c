...
static int ltr559_read_data_for_cali(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali)
{
	...
	for(i = 0;i<20;i++)
	{
		...
		if(!ltr559_ps_read(client,&data[i]))
		{
			sum += data[i];
	...
	{
		data_cali = sum/20;
		//APS_LOG("[hjf++]ltr559_read_data_for_cali data_cali = %d\n",data_cali);
		ps_data_cali->far_away = data_cali + CUST_LTR559_PS_THRES_FAR;
		ps_data_cali->close = data_cali + CUST_LTR559_PS_THRES_CLOSE;
	...
	else
	{
            	...
            	//APS_LOG("ltr559_read_data_for_cali get data error!\n");
            	err=  -1;
	}
...
static void ltr559_WriteCalibration(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *data_cali)
{
	struct ltr559_priv *obj = i2c_get_clientdata(client);
	struct alsps_hw *hw = get_cust_alsps();
	const char *name = "mediatek,ltr559";
	APS_FUN();
    APS_LOG("ltr559_WriteCalibration() ...start!\n");
       ...
       unsigned long arg)       
{
	...
	else
	{
        ...
        case ALSPS_SET_PS_CALI:
            ...
            ltr559_WriteCalibration(obj->client,&ps_cali_temp);
	...
	ltr559_ps_enable(obj->client, 1);
	ltr559_read_data_for_cali(obj->client,&ps_cali_temp);
