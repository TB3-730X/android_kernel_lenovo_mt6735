...
static int __init dt_get_boot_reason(unsigned long node, const char *uname, int depth, void *data)
{
	...
	if (emmc_ptr) {
		if ((emmc_name_ptr = strstr(emmc_ptr, "emmc_name=")) != 0) {
			emmc_name_ptr += 10;
			if((emmc_name_ptr - emmc_ptr) > strlen(emmc_ptr+1)) {
				pr_warn("[wt]error: can't find emmc name in cmdline");
			}
			q = emmc_name_ptr;
			while(*q != ' ' && *q != '\0') q++;
			...
			pr_warn("[wt] cmdline emmc_flash info = %s\n",emmc_device_name);
		} else {
			pr_warn("[wt]cannot find emmc name in cmdline\n");
		}
		pr_warn("%s\n", emmc_ptr);
	} else
		pr_warn("'bootargs' is not found\n");
