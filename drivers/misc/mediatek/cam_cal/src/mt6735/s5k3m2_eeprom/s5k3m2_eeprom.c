...
static long s5k3m2_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	CAM_CALDB("[CAMERA SENSOR] s5k3m2_OTP_DEVICE_ID,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
...
extern uint8_t s5k3m2_lsc[1868];
extern uint8_t s5k3m2_awb[16];
extern uint8_t ModuleHouseID;

static int selective_read_region(u32 offset, BYTE* data,u32 size)
{
    if (offset == 0xFFFFFFFA) {//s5k3m2
        *data = ModuleHouseID;
        return size;
    } else if (offset == 0xFFFFFFFB){//lsc
        CAM_CALDB("get s5k3m2 lenc");        
        if (size != 1868) {
            CAM_CALERR("[CAMERA SENSOR] s5k3m2 lencs ize is out of range\n");
            return -EBADTYPE;
        }
        memcpy((void *)data,(void *)&s5k3m2_lsc,size);
        return size;
    } else if (offset == 0xFFFFFFFC){//awb
        CAM_CALDB("get s5k3m2 awb");        
        if (size != 16) {
            CAM_CALERR("[CAMERA SENSOR] s5k3m2 awb size is out of range\n");
            return -EBADTYPE;
        }
        memcpy((void *)data,(void *)&s5k3m2_awb,size);
        return size;
    }  else {
