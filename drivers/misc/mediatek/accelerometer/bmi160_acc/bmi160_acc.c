...
static int bmi160_acc_remap_to_gravity(struct i2c_client *client, int data[BMI160_ACC_AXES_NUM]);
...
/*----------------------------------------------------------------------------*/
/*--------------------------------wt add for gs cali start--------------------------------------------*/
static int BMI160_ACC_WriteCalibration_New(struct i2c_client *client, struct GS_CALI_DATA_STRUCT *data_cali)
{
	struct bmi160_acc_i2c_data *obj = obj_i2c_data;
...
{
	struct bmi160_acc_i2c_data *obj = obj_i2c_data;
	int acc_tmp[BMI160_ACC_AXES_NUM]={0};
	if(NULL == client)
	{
		return -EINVAL;
	}
	/*remap coordinate*/
	acc_tmp[obj->cvt.map[BMI160_ACC_AXIS_X]] = obj->cvt.sign[BMI160_ACC_AXIS_X]* data[BMI160_ACC_AXIS_X];
	acc_tmp[obj->cvt.map[BMI160_ACC_AXIS_Y]] = obj->cvt.sign[BMI160_ACC_AXIS_Y]* data[BMI160_ACC_AXIS_Y];
	acc_tmp[obj->cvt.map[BMI160_ACC_AXIS_Z]] = obj->cvt.sign[BMI160_ACC_AXIS_Z]* data[BMI160_ACC_AXIS_Z];
	
	//Out put the mg
	data[BMI160_ACC_AXIS_X] = acc_tmp[BMI160_ACC_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[BMI160_ACC_AXIS_Y] = acc_tmp[BMI160_ACC_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[BMI160_ACC_AXIS_Z] = acc_tmp[BMI160_ACC_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
...
static long bmi160_acc_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	/*-----wt add for gs cali start ---*/
	int acc_tmp[BMI160_ACC_AXES_NUM] = {0};
	s16 raw_data[BMI160_ACC_AXES_NUM] = {0};
	uint32_t enable;
	...
	/*------------------------------wt add for gs cali end----------------------------------------------*/
		...
		case GSENSOR_IOCTL_READ_RAW_DATA:
			...
			*/
			/*------------------------------wt add for gs cali start----------------------------------------------*/
			BMI160_ACC_ReadData(client, raw_data);
			
				acc_tmp[0] = (int)raw_data[0];
				acc_tmp[1] = (int)raw_data[1];
				acc_tmp[2] = (int)raw_data[2];
			
				err = bmi160_acc_remap_to_gravity(client, acc_tmp);
		...
		case GSENSOR_IOCTL_SET_CALI_NEW:
			...
			{/* get remapped value in NV (mg) */
				err = BMI160_ACC_WriteCalibration_New(client, &gs_cali_temp);
				if(err){
					err = -EFAULT;
					break;
				}
...
static long bmi160_acc_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_GSENSOR_IOCTL_READ_SENSORDATA:
		ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_READ_SENSORDATA,
				(unsigned long)arg32);
		if (ret) {
			GSE_ERR("COMPAT_GSENSOR_IOCTL_READ_SENSORDATA unlocked_ioctl failed.");
			return ret;
		}
		break;
	case COMPAT_GSENSOR_IOCTL_SET_CALI:
		...
		ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_SET_CALI,
				(unsigned long)arg32);
		if (ret) {
			GSE_ERR("COMPAT_GSENSOR_IOCTL_SET_CALI unlocked_ioctl failed.");
			return ret;
		}
		break;
	case COMPAT_GSENSOR_IOCTL_CLR_CALI:
		...
		ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_CLR_CALI,
				(unsigned long)(arg32));
		if (ret) {
			GSE_ERR("COMPAT_GSENSOR_IOCTL_CLR_CALI unlocked_ioctl failed.");
			return ret;
		}
		break;
	case COMPAT_GSENSOR_IOCTL_GET_CALI:
		...
		ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_GET_CALI,
				(unsigned long)(arg32));
		if (ret) {
			GSE_ERR("COMPAT_GSENSOR_IOCTL_GET_CALI unlocked_ioctl failed.");
			return ret;
		}
		break;
	case COMPAT_GSENSOR_IOCTL_SET_CALI_NEW:
		...
		ret = file->f_op->unlocked_ioctl(file, GSENSOR_IOCTL_SET_CALI_NEW,
				(unsigned long)(arg32));
		if (ret) {
			GSE_ERR("COMPAT_GSENSOR_IOCTL_SET_CALI_NEW unlocked_ioctl failed.");
			return ret;
		}
		break;
	default:
