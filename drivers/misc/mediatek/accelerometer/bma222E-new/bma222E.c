...
static int bma222_remap_to_gravity(struct i2c_client *client, int data[BMA222_AXES_NUM]);
...
/*--------------------------------wt add for gs cali start--------------------------------------------*/
static int BMA222_WriteCalibration_New(struct i2c_client *client, struct GS_CALI_DATA_STRUCT *data_cali)
{
...
static int BMA222_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if (res != 0) {
	...
	else
		{
			...
			/* compensate data */
			if(gs_cali.valid == 1){
				acc[BMA222_AXIS_X] -= gs_cali.x;
				acc[BMA222_AXIS_Y] -= gs_cali.y;
				acc[BMA222_AXIS_Z] -= gs_cali.z;
			}
...
static int bma222_remap_to_gravity(struct i2c_client *client, int data[BMA222_AXES_NUM])
{
	...
	/*remap coordinate*/
	acc_tmp[obj->cvt.map[BMA222_AXIS_X]] = obj->cvt.sign[BMA222_AXIS_X]* data[BMA222_AXIS_X];
	acc_tmp[obj->cvt.map[BMA222_AXIS_Y]] = obj->cvt.sign[BMA222_AXIS_Y]* data[BMA222_AXIS_Y];
	acc_tmp[obj->cvt.map[BMA222_AXIS_Z]] = obj->cvt.sign[BMA222_AXIS_Z]* data[BMA222_AXIS_Z];

	//Out put the mg
	data[BMA222_AXIS_X] = acc_tmp[BMA222_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[BMA222_AXIS_Y] = acc_tmp[BMA222_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
	data[BMA222_AXIS_Z] = acc_tmp[BMA222_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
...
static long bma222_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	/*-----wt add for gs cali start ---*/
	int acc_tmp[BMA222_AXES_NUM] = {0};
	s16 raw_data[BMA222_AXES_NUM] = {0};
	uint32_t enable;
...
/*------------------------------wt add for gs cali start----------------------------------------------*/
	case GSENSOR_SET_ACC_MODE:
		...
		if(enable == 1)
			err =BMA222_SetPowerMode(client,BMA_NORMAL_MODE);
		else
			err = BMA222_SetPowerMode(client,BMA_SUSPEND_MODE);
		if (err)
...
/*------------------------------wt add for gs cali end----------------------------------------------*/
	...
	case GSENSOR_IOCTL_READ_SENSORDATA:
		data = (void __user *)arg;
		if (data == NULL) {
			GSE_ERR("GSENSOR_IOCTL_READ_SENSORDATA  data is null err!!\n");
			err = -EINVAL;
			break;
		}
		...
		BMA222_ReadSensorData(client, strbuf, BMA222_BUFSIZE);
		if (copy_to_user(data, strbuf, strlen(strbuf) + 1)) {
			GSE_ERR("GSENSOR_IOCTL_READ_SENSORDATA  copy_to_user err!!\n");
			err = -EFAULT;
			break;
		}
		break;
	...
	case GSENSOR_IOCTL_READ_RAW_DATA:
		data = (void __user *)arg;
		if (data == NULL) {
			GSE_ERR("GSENSOR_IOCTL_READ_RAW_DATA  data is null err!!\n");
			err = -EINVAL;
			break;
		}
		/*------------------------------wt add for gs cali start----------------------------------------------*/
		BMA222_ReadData(client, raw_data);
			acc_tmp[0] = (int)raw_data[0];
			acc_tmp[1] = (int)raw_data[1];
			acc_tmp[2] = (int)raw_data[2];

			err = bma222_remap_to_gravity(client, acc_tmp);
			if(err){
				err = -EFAULT;
				break;
			}
	...
	case GSENSOR_IOCTL_SET_CALI:
		...
		case GSENSOR_IOCTL_SET_CALI_NEW:
			...
			{/* get remapped value in NV (mg) */
				err = BMA222_WriteCalibration_New(client, &gs_cali_temp);
				if(err){
					err = -EFAULT;
					break;
				}
