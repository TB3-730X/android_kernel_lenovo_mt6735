...
static ssize_t st480_selftest(struct device_driver *ddri, char *buf)
{
	int ret;
	int sf_ret = 0;
	char result[10];
...
/*self test*/
	...
	while (st480_i2c_transfer_data(st480->client, 4, buffer, 1) != 0) {
		...
		if (ret > MAX_FAILURE_COUNT) {
			sf_ret = -1;
			MSE_ERR("[SENSOR]: %s i2c transfer data error\n",__func__);
		}
	}
	...
	MSE_ERR("[SENSOR]: %s zh[0] = %d, zh[1] = %d, zh[2] = %d\n",
...
/*set mode*/
	...
	if ((abs(zh[2]) <= 1) || (abs(zh[2]) >= 254)) {
		sf_ret = -1;
		MSE_ERR("[SENSOR]: %s BIST test error\n", __func__);
	}
	if (sf_ret == 0){
		MSE_ERR("[SENSOR]: %s success\n", __func__);
		strcpy(result,"y");
	}
	else{
		MSE_ERR("[SENSOR]: %s fail\n", __func__);
		strcpy(result,"n");
	}
	if (atomic_read(&st480->enable) == 1) {
...
static DRIVER_ATTR(selftest, S_IRUGO | S_IWUSR, st480_selftest, NULL);
/*----------------------------------------------------------------------------*/
...
static long st480d_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:
		...
		hwm_data->values[0] = mag.mag_x;
		hwm_data->values[1] = mag.mag_y;
		hwm_data->values[2] = mag.mag_z;
		...
		//printk("quanzhen %d,%d,%d,%d,%d\n",hwm_data->values[0],hwm_data->values[1],hwm_data->values[2],hwm_data->status,hwm_data->value_divide);
