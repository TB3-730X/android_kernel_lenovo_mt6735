...
static DEFINE_MUTEX(leds_gpio_mutex);
#endif
...
#if defined(CONFIG_MTK_NCP1851_SUPPORT)
//Other guanyuwei@wt 150106 add brightness set by gpio for red led
extern struct pinctrl *redled_pinctrl;
extern struct pinctrl_state *gpio55_state_of_low;
extern struct pinctrl_state *gpio55_state_of_high;

int brightness_set_gpio(enum mt65xx_led_pmic pmic_type, u32 level)
{
	LEDS_DEBUG("PMIC# %s %d:%d\n", __func__, pmic_type, level);
	mutex_lock(&leds_gpio_mutex);
	if (level != 0) {
		pinctrl_select_state(redled_pinctrl, gpio55_state_of_low);
	} else if (level == 0) {
		pinctrl_select_state(redled_pinctrl, gpio55_state_of_high);
	}
	mutex_unlock(&leds_gpio_mutex);
	return 0;
}
//add end
#elif defined(CONFIG_MTK_BQ24296_SUPPORT)
extern int mt_led_set_gpio(unsigned int level);
#endif
