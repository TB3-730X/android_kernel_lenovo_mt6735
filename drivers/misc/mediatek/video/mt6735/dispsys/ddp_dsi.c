...
static void _DSI_INTERNAL_IRQ_Handler(DISP_MODULE_ENUM module, unsigned int param)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (status.RD_RDY) {
			...
			waitRDDone = true;
			cmdqCoreSetEvent(CMDQ_SYNC_TOKEN_POLL_ESD);
			DSI_OUTREGBIT(NULL, struct DSI_INT_STATUS_REG,
					      DSI_REG[0]->DSI_INTSTA, RD_RDY, 0);
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	if (state == CMDQ_BEFORE_STREAM_SOF) {
		...
		for (i = 0; i < 3; i++) {
			...
			if (dsi_i == 0) {	/* DSI0 */
				//cmdqRecWait(cmdq_trigger_handle, CMDQ_SYNC_TOKEN_POLL_ESD);
				cmdqRecWaitNoClear(cmdq_trigger_handle, CMDQ_SYNC_TOKEN_POLL_ESD);
				cmdqRecClearEventToken(cmdq_trigger_handle,  CMDQ_SYNC_TOKEN_POLL_ESD);
