...
static ssize_t sx9310_register_read_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	u8 reg_value = 0; 
	u8 i=0;
	u8 reg_address;
	psx93XX_t this = dev_get_drvdata(dev);
	dev_dbg(this->pdev, "Reading register\n");
...
static DEVICE_ATTR(sx9310_reg, 0664,sx9310_register_read_store, sx9310_register_write_store);
static DEVICE_ATTR(calibrate, 0664, manual_offset_calibration_show,  manual_offset_calibration_store);

static struct device_attribute *sx9310_attributes[] = {
...
static void hw_init(psx93XX_t this)
{
	...
	/* configure device */
	pr_err("Going to Setup I2C Registers\n");
      ...
      switch (pCurrentButton->state) {
        case IDLE: /* Button is not being touched! */
          if (((i & pCurrentButton->mask) == pCurrentButton->mask)) {
            /* User pressed button */
            pr_err( "IDLE cap button %d touched %d keycode\n", counter,pCurrentButton->keycode);
            input_report_key(input, pCurrentButton->keycode, 1);
            pCurrentButton->state = ACTIVE;
          } else {
        ...
        case ACTIVE: /* Button is being touched! */ 
          if (((i & pCurrentButton->mask) != pCurrentButton->mask)) {
            ...
            pr_err( "cap button %d released %d  pCurrentButton->keycode\n",counter,pCurrentButton->keycode);
