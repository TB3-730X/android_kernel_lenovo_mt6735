...
extern char lcm_name_for_tp[256];
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *path)
{
	...
	if(GTP_FW_SENSOR_ID == 0x08 || FW_VERSION_ID == 0x08){
			       ...
			       update_msg.fw_total_len = sizeof(BOE_FW) - FW_HEAD_LENGTH;
	}
			       ...
			       GTP_INFO("Update by QUNCHUANG FW,Sensor ID : %d",GTP_FW_SENSOR_ID);
			        gtp_default_FW = QUNCHUANG_FW;
			         update_msg.fw_total_len = sizeof(QUNCHUANG_FW) - FW_HEAD_LENGTH;
        }else{
			       GTP_INFO("Force Upgrade !");
			       if (strcmp(lcm_name_for_tp, "hx8394d_wsvga_dsi_guanzhen_vdo") == 0){
			                  GTP_INFO("Force Upgrade For BOE !");
					  gtp_default_FW = BOE_FW;
			                  update_msg.fw_total_len = sizeof(BOE_FW) - FW_HEAD_LENGTH;		  
			       	}else if(strcmp(lcm_name_for_tp, "nt51021b_wsvga_dsi_vdo_innolux") == 0){
				   	 GTP_INFO("Force Upgrade For Qunchuang !");
			        	 gtp_default_FW = QUNCHUANG_FW;
			        	 update_msg.fw_total_len = sizeof(QUNCHUANG_FW) - FW_HEAD_LENGTH;
			       	}else{
	...
	GTP_INFO("gtp_default_FW:%d",(update_msg.fw_total_len + FW_HEAD_LENGTH));
        if ((update_msg.fw_total_len + FW_HEAD_LENGTH) < (FW_HEAD_LENGTH + FW_SECTION_LENGTH*4+FW_DSP_ISP_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH))
        {
            GTP_INFO("[check_update_file]default firmware array is INVALID!");
            return FAIL;
        }
		
        GTP_INFO("Firmware actual size: %d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);
        memcpy(fw_head, &gtp_default_FW[0], FW_HEAD_LENGTH);
       
        GTP_INFO("check firmware legality");
        fw_checksum = 0;
	 ...
	 GTP_INFO("check firmware legality is ok");
        return SUCCESS;
...
static u8 gup_burn_fw_finish(struct i2c_client *client)
{
    ...
    //step1:alloc memory
    GTP_INFO("[burn_fw_finish]step1:alloc memory");
    while(retry++ < 5)
    {
        fw_ss51 = (u8*)kzalloc(FW_SECTION_LENGTH, GFP_KERNEL);
        if(fw_ss51 == NULL)
    ...
    if(retry >= 5)
    {
        GTP_INFO("[burn_fw_finish]Alloc memory fail,exit.");
        return FAIL;
    }
    
    GTP_INFO("[burn_fw_finish]step2: burn ss51 first 8K.");
    ret = gup_load_section_file(fw_ss51, 0, FW_SECTION_LENGTH, SEEK_SET);
    if(FAIL == ret)
    {
    ...
    GTP_INFO("[burn_fw_finish]step3:clear control flag");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, 0x00);
    if(ret <= 0)
    {
        GTP_ERROR("[burn_fw_finish]clear control flag fail.");
    ...
    GTP_INFO("[burn_fw_finish]step4:burn ss51 firmware section 1");
    ret = gup_burn_fw_section(client, fw_ss51, 0xC000, 0x01);
    if(FAIL == ret)
    {
    ...
    GTP_INFO("[burn_fw_finish]step5:enable download DSP code ");
    ...
    GTP_INFO("[burn_fw_finish]step6:release ss51 & hold dsp");
...
s32 gup_update_proc(void *dir)
{
    ...
    if (FAIL == ret)
    {
        GTP_INFO("[update_proc]enter update mode fail.");
        goto update_fail;
    }
...
update_fail:
    ...
    GTP_INFO("[update_proc]leave update mode:%d",update_ret);
