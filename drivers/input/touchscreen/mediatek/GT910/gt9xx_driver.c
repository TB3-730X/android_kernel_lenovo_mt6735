...
#if GTP_BIGHAND_SUPPORT
u8 large_touch_mask = 0;  
#endif
int touch_irq;
static int tpd_flag;
int tpd_halt = 0;
...
extern void Ctp_CmcsTest_Proc_init(void);
#endif

#if WT_CTP_GESTURE_SUPPORT
extern char gtp_gesture_value;
extern char gtp_gesture_onoff;
extern void Ctp_Gesture_Fucntion_Proc_File(void);
#endif


#if  WT_CTP_INFORMATION
...
static char tp_string_version[40];
 ...
 static unsigned char BOE_FW[]= 
{
#include "FW_BOE.txt"
};

static unsigned char QUNCHUANG_FW[]= 
{
#include "FW_QUNCHUANG.txt"
};
#endif
...
#if GTP_ESD_PROTECT
static int clk_tick_cnt = 200;                  /*ESD:2s*/
...
static ssize_t gt91xx_config_read_proc(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	char *ptr = buffer;
	...
	ptr +=
       ...
       gtp_i2c_read_dbl_check(i2c_client_point, GTP_REG_SENSOR_ID, &GTP_FW_SENSOR_ID, 1);
	if(GTP_FW_SENSOR_ID == 0x08 ||FW_VERSION_ID == 0x08 ){
					  GTP_INFO("Update by BOE FW,Sensor ID : %d",GTP_FW_SENSOR_ID);
					  gtp_default_FW = BOE_FW;
	   }
	 else if(GTP_FW_SENSOR_ID == 0x02 ||FW_VERSION_ID == 0x02){
					  GTP_INFO("Update by QUNCHUANG FW,Sensor ID : %d",GTP_FW_SENSOR_ID);
					   gtp_default_FW = QUNCHUANG_FW;
        }else{
                                       GTP_INFO("Unknow Sensor ID : %d",GTP_FW_SENSOR_ID);                       
	  }      
...
s32 gtp_send_cfg(struct i2c_client *client)
{
	...
	for (retry = 0; retry < 5; retry++) {
		...
		if (ret > 0){
			GTP_INFO("Write normal config done");
			break;
			}
	}
...
//	u8 rd_cfg_buf[16];
	...
	GTP_INFO("len1=%d,len2=%d,len3=%d,get_len=%d", cfg_info_len[0],
	...
	if ((!cfg_info_len[1]) && (!cfg_info_len[2])) {
	 ...
	 ret = gtp_i2c_read_dbl_check(client, GTP_REG_SENSOR_ID, &sensor_id, 1);
       ...
       FW_VERSION_ID = sensor_id;
       if(sensor_id == 0x08)
	   	 sensor_id = 0x01;

       	ret = i2c_read_bytes(client, GTP_REG_CONFIG_DATA, &temp_data, 1);
	if (ret < 0) {
	...
	version_id=sensor_id;
	GTP_INFO("SENSOR ID:%d,IC config version: 0x%x,cfg_len: %d\n",version_id,temp_data,cfg_len);
	...
	memcpy(&config_charger[GTP_ADDR_LENGTH],
	       send_cfg_buf_charger[sensor_id], cfg_len);
#endif
	...
	{
#if GTP_DRIVER_SEND_CFG
                GTP_INFO("Send config ");
		ret = gtp_send_cfg(client);

		if (ret < 0) {
			GTP_INFO("Send config error.");
		}
#endif
...
reset_proc:
	...
	//hwPowerOn(MT65XX_POWER_LDO_VGP2, VOL_3300, "TP");
	 tpd_gpio_output(GTP_VDD_PORT, 1);
#endif
...
/*other,lujian.wt,get the lcm name for tp force upgrade*/
char *  mtkfb_find_lcm_for_tp(void)
{
		char *p, *q;
		...
		memset((void *)lcm_name_for_tp, 0, sizeof(lcm_name_for_tp));
		strncpy((char *)lcm_name_for_tp, (const char *)p, (int)(q - p));
		lcm_name_for_tp[q - p + 1] = '\0';

	         printk("%s, %s\n", __func__, lcm_name_for_tp); 
	        return lcm_name_for_tp;
}
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	gt91xx_config_proc =
	    proc_create(GT91XX_CONFIG_PROC_FILE, 0666, NULL, &gt_upgrade_proc_fops);
	...
	mtkfb_find_lcm_for_tp();                                                        /*get the lcd name for tp force upgrader*/      
  ...
  if (FACTORY_BOOT == get_boot_mode() || RECOVERY_BOOT == get_boot_mode()){
  	  GTP_ERROR("Don't update in Factory_boot and Recovery_boot.");
  }else{
 ...
 Ctp_Gesture_Fucntion_Proc_File();
      ...
      if(version_id==0)
	{
		sprintf(tp_string_version,"BOE,GT9293_%x,V%d",  version_info,cfg_version);
	}
	...
	{
		sprintf(tp_string_version,"QunChuang,GT9293_%x,V%d",  version_info,cfg_version);
	}
	else
	{
		sprintf(tp_string_version,"XXX,GT9293_%x,V%d",  version_info,cfg_version);
	}
      
	hardwareinfo_set_prop(HARDWARE_TP, tp_string_version);
	GTP_INFO(TPD_DEVICE " GT9293_%x,V%d", version_info,cfg_version);
#endif
...
static void force_reset_guitar(void)
{
	...
	tpd_gpio_output(GTP_VDD_PORT, 0);
#endif
#ifdef TPD_POWER_SOURCE_1800
...
//	u8 test[3] = { GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff };
		   ...
		   if ((esd_buf[0] == 0xAA) || (esd_buf[1] != 0xAA))
	            {
	                ...
	                if ( (chk_buf[0] == 0xAA) || (chk_buf[1] != 0xAA) )
	                {
	                    ...
	                    GTP_INFO("[Esd] jump to reset guitar");
	                    break;
	                }
	                else
	                {
	            ...
	            else
	            {
	                ...
	                GTP_DEBUG("[Esd] IC works normally, Write 0x8040 0xAA ");
	                break;
	            }
		}
	}
	...
	if (i >= 3) {
		...
		if (1 == rqst_processing) {
			GTP_DEBUG("Request Processing, no reset guitar...");
		} else
#endif
		{
#if GTP_COMPATIBLE_MODE
			esd_resetting = 1;
#endif
            GTP_DEBUG("IC works abnormally! Process reset guitar.");
            esd_buf[0] = 0x01;
            esd_buf[1] = 0x01;
            esd_buf[2] = 0x01;
	...
	if (!tpd_halt) {
		GTP_DEBUG("[TP ESD] queue_delayed_work AGAIN ");
...
static int touch_event_handler(void *unused)
{
	...
	do {
		...
		if (DOZE_ENABLED == doze_status)
		{
		        GTP_INFO("Gesture enable type;%d\n",doze_status);
			ret = gtp_i2c_read(i2c_client_point, doze_buf, 3);
			GTP_DEBUG("0x814B = 0x%02X", doze_buf[2]);
			if (ret > 0)
			{				
				if ((doze_buf[2] == 0x63) || (doze_buf[2] == 0x65) || (doze_buf[2] == 0x6D) ||
					(doze_buf[2] == 0x6F) || (doze_buf[2] == 0x73) || (doze_buf[2] == 0x77) || 
					(doze_buf[2] == 0x7A) || (doze_buf[2] == 0x76) ||
					(doze_buf[2] == 0x5E) /* ^ */
					)
				{
   ...
   if ((finger & 0x40) == 0x40) 
		...
		if (touch_num) {
			for (i = 0; i < touch_num; i++) {
				...
				input_y = 1024 - TPD_WARP_Y(abs_y_max, input_y);
