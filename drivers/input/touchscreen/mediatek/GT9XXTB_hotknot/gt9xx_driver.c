...
s32 gtp_send_cfg(struct i2c_client *client)
{
	...
	GTP_DEBUG("Driver Send Config OK !");
#endif
	return ret;
}
...
static s32 gtp_init_panel(struct i2c_client *client)
{
...
#endif
	{
		...
		if (ret == SUCCESS) {
			...
			if (flash_cfg_version < 90 && flash_cfg_version > grp_cfg_version) { 
	...
	do {
		...
		if (touch_num) {
			for (i = 0; i < touch_num; i++) {
				...
				GTP_DEBUG("abs_x_max:%04d,abs_y_max:%04d",abs_x_max, abs_y_max);
