...
static char tp_string_version[40];
 ...
 static unsigned char OUFEI_FW[]= 
{
#include "FW_OUFEI.txt"
};

#endif

#if GTP_HAVE_TOUCH_KEY
static   int touch_key_array[] = TPD_KEYS;
...
static ssize_t gt91xx_config_read_proc(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if(GTP_FW_SENSOR_ID == 0x0 ||FW_VERSION_ID == 0x0 ){
					  GTP_INFO("Update by OUFEI FW,Sensor ID : %d",GTP_FW_SENSOR_ID);
					  gtp_default_FW = OUFEI_FW;
	   }else{
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
 ...
 Ctp_Gesture_Fucntion_Proc_File();
      ...
      if(version_id==0)
	{
		sprintf(tp_string_version,"OUFEI,GT9271_%x,V%d",  version_info,cfg_version);
	}
	else
	{
		sprintf(tp_string_version,"XXX,GT9271_%x,V%d",  version_info,cfg_version);
	}
      
	hardwareinfo_set_prop(HARDWARE_TP, tp_string_version);
	GTP_INFO(TPD_DEVICE " GT9271_%x,V%d", version_info,cfg_version);
#endif
...
//	u8 test[3] = { GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff };
	...
	if (tpd_halt)
       ...
       GTP_INFO("[TP ESD] gtp_esd_check_func");
	for (i = 0; i < 3; i++) {
		ret = i2c_read_bytes(i2c_client_point, 0x8040,esd_buf, 2);
               GTP_INFO("[Esd]0x8040 = 0x%02X, 0x8041 = 0x%02X", esd_buf[0], esd_buf[1]);
		if (ret > 0){
		     // IIC communication problem
		     GTP_INFO("[Esd] IIC communication problem ");
                     continue;
		} 
		else{
	...
	if (!tpd_halt) {
		GTP_INFO("[TP ESD] queue_delayed_work AGAIN ");
	...
	if (tpd_dts_data.use_tpd_button) {
		/*initialize tpd button data */
          /* for (idx = 0; idx < GTP_MAX_KEY_NUM; idx++)
	        ...
	        tpd_button_setting(TPD_KEY_COUNT, touch_key_array, tpd_keys_dim_local);
	}
#if (defined(TPD_WARP_START) && defined(TPD_WARP_END))
	TPD_DO_WARP = 1;
