...
struct pinctrl_state *eint_as_int, *eint_output0, *eint_output1, *rst_output0, *rst_output1,*vdd_output0, *vdd_output1;
...
int tpd_get_gpio_info(struct platform_device *pdev)
{
	...
	vdd_output0 = pinctrl_lookup_state(pinctrl1, "state_vdd_output0");
	if (IS_ERR(vdd_output0)) {
		ret = PTR_ERR(vdd_output0);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_vdd_output0!\n");
		return ret;
	}
	vdd_output1 = pinctrl_lookup_state(pinctrl1, "state_vdd_output1");
	if (IS_ERR(vdd_output1)) {
		ret = PTR_ERR(vdd_output1);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_vdd_output1!\n");
		return ret;
	}
	TPD_DEBUG("[tpd%d] mt_tpd_pinctrl----------\n", pdev->id);
	return 0;
