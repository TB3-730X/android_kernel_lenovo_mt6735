...
static struct work_struct escore_eint_work;
static struct workqueue_struct *escore_eint_workqueue;
...
extern unsigned int GetI2SAdcSwitch(void);
extern bool SetI2SAdcSwitch(bool enable);

static int es705_audio_adc_switch_get_value(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = GetI2SAdcSwitch();
	dev_dbg(escore_priv.dev, "%s(): Audio Adc Switch = %d\n",
		__func__, ucontrol->value.integer.value[0]);

	return 0;
}

static int es705_audio_adc_switch_put_value(struct snd_kcontrol *kcontrol,
				       struct snd_ctl_elem_value *ucontrol)
{
	if (GetI2SAdcSwitch() !=
			ucontrol->value.integer.value[0]) {
		SetI2SAdcSwitch(ucontrol->value.integer.value[0]);
	}

	dev_dbg(escore_priv.dev, "%s(): Audio Adc Switch = %d\n",
		__func__, GetI2SAdcSwitch());
...
static struct snd_kcontrol_new es705_digital_ext_snd_controls[] = {
	...
	SOC_SINGLE_EXT("SDE RMS Estimate Value", ES80X_SDE_RMS_ESTIMATE_VALUE,
			...
			NULL),
	//csq.modify.bug156979.P98998AA1.耳机通话，通话端无声
	SOC_SINGLE_EXT("Audio Adc Switch", SND_SOC_NOPM, 0, 1, 0,
		es705_audio_adc_switch_get_value,
		es705_audio_adc_switch_put_value),
	//csq.modify.bug156979.P98998AA1.耳机通话，通话端无声end
};
...
int remote_add_codec_control(void *ptr)
{
	...
	dev_info(codec->dev,"%s(): start add control, escore->flag.fw_download_init_done %d\n",__func__, escore->flag.fw_download_init_done);

	if (!escore->flag.fw_download_init_done) {
		init_completion(&escore->fw_download);
		escore->flag.fw_download_init_done = 1;
	}
	...
	dev_info(codec->dev,"%s(): end add control\n",__func__);
...
err_pm_get_sync:
	...
	queue_work(escore_eint_workqueue, &escore_eint_work);
	enable_irq(irq);
	return IRQ_HANDLED;
}

static int escore_eint_work_callback(struct work_struct *work)
{
#if 1
	struct escore_priv *escore = &escore_priv;
	int rc;
...
static struct esxxx_platform_data *es705_populate_dt_pdata(struct device *dev)
{
	...
	u32 val = 0;
	u32 debounce[4] = { 0, 0 , 0, 0 };
	u32 interrupts[4] = { 0, 0 , 0, 0 };
	int ret;
	...
	pdata->es_pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(pdata->es_pinctrl)) {
		ret = PTR_ERR(pdata->es_pinctrl);
		dev_err(dev, "Cannot find es_pinctrl!\n");
		return ret;
	}

	pdata->pins_es_pins_mode0 = pinctrl_lookup_state(pdata->es_pinctrl, "es814pins-mode0");
	if (IS_ERR(pdata->pins_es_pins_mode0)) {
		ret = PTR_ERR(pdata->pins_es_pins_mode0);
		dev_err(dev, "Cannot find es814pins-mode0!\n");
		return ret;
	}
	pinctrl_select_state(pdata->es_pinctrl, pdata->pins_es_pins_mode0);

	pdata->pins_es_pins_mode1 = pinctrl_lookup_state(pdata->es_pinctrl, "es814pins-mode1");
	if (IS_ERR(pdata->pins_es_pins_mode1)) {
		ret = PTR_ERR(pdata->pins_es_pins_mode1);
		dev_err(dev, "Cannot find es814pins-mode1!\n");
		return ret;
	}
	pinctrl_select_state(pdata->es_pinctrl, pdata->pins_es_pins_mode1);

	dev_info(dev, "set clkm1 output 26M!\n");
	clk_monitor(1, 1, 0);	
	...
	if (pdata->reset_gpio < 0) {
		dev_err(dev, "%s(): get es814_rst_gpio failed\n", __func__);
	...
	of_property_read_u32_array(dev->of_node, "interrupts", interrupts, ARRAY_SIZE(interrupts));
	of_property_read_u32_array(dev->of_node, "debounce", debounce, ARRAY_SIZE(debounce));

	dev_info(dev, "interrupts: %d %d %d %d\n", interrupts[0], interrupts[1], interrupts[2], interrupts[3]);
	dev_info(dev, "debounce: %d %d %d %d\n", debounce[0], debounce[1], debounce[2], debounce[3]);

/* API Interrupt registration */
#ifdef CONFIG_SND_SOC_ES_GPIO_A
	dev_dbg(dev, "%s(): gpioa configured\n", __func__);
	pdata->gpioa_gpio = interrupts[2];
	pdata->gpio_a_irq_type = ES_FALLING_EDGE;
	pdata->gpio_a_interrupts = irq_of_parse_and_map(dev->of_node, 1);
	...
	pdata->gpiob_gpio = interrupts[0];
	pdata->gpio_b_irq_type = ES_RISING_EDGE;
	pdata->gpio_b_interrupts = irq_of_parse_and_map(dev->of_node, 0);
	dev_info(dev, "pdata->gpio_b_interrupts %d\n", pdata->gpio_b_interrupts);
...
int es705_core_probe(struct device *dev)
{
	...
	const char *fw_filename = "audience-es814-fw.bin";
	const char *vs_filename = "audience-es814-vs.bin";
	const char *bkg_filename = "adnc_cvq_detection_bkg_w_hdrs.bin";
	const char *fw_af_filename = "audience-es814-af.bin";
#else
...
*/
	...
	if (rc) {
		...
		dev_info(escore_priv.dev, "%s(): request_firmware(%s) success %d\n",
			__func__, fw_filename, rc);
	}
#if defined(CONFIG_SND_SOC_ES_VS) && \
	...
	/* API Interrupt registration */
	if (pdata->gpioa_gpio != -1) {
		rc = request_irq(pdata->gpio_a_interrupts, es705_cmd_completion_isr, IRQF_TRIGGER_FALLING, "es705-cmd-completion-isr", &escore_priv);
		if (rc < 0) {
	...
	if (pdata->gpiob_gpio != -1) {
		rc = request_irq(pdata->gpio_b_interrupts, es705_irq_work, IRQF_TRIGGER_RISING | IRQF_ONESHOT, "es705_irq_work", &escore_priv);
		if (rc) {
			dev_err(escore_priv.dev,
			...
			dev_info(escore_priv.dev,"%s(): event request_irq() success\n",__func__);
			escore_eint_workqueue = create_singlethread_workqueue("escore_eint");
			INIT_WORK(&escore_eint_work, escore_eint_work_callback);
		}

		/* Disable the interrupt till needed */
		if (pdata->gpio_b_interrupts != -1)
			disable_irq_nosync(pdata->gpio_b_interrupts);
...
static __init int es705_init(void)
{
	...
	if (!escore_priv.flag.fw_download_init_done) {
		init_completion(&escore_priv.fw_download);
		escore_priv.flag.fw_download_init_done = 1;
	}
#if defined(CONFIG_SND_SOC_ES_ASYNC_FW_LOAD)
