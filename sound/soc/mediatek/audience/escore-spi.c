 ...
 */
	...
	if (is_vmalloc_or_module_addr(buf)) {
		...
		dev_info(escore->dev, "Alignment non_dma_bytes[%d], dma_bytes[%d], PAGE_SIZE[%d], len[%d]\n", 
			non_dma_bytes, dma_bytes, PAGE_SIZE, len);
		...
		if (non_dma_bytes) {
			...
			current_vaddr = kaddr;
			while (non_dma_bytes > 0) {
				rc = spi_write(escore_spi, current_vaddr, 
					(non_dma_bytes > ES_SPI_DMA_MIN_BYTES) ?
					ES_SPI_DMA_MIN_BYTES : non_dma_bytes);
				if (rc < 0) {
					dev_err(escore->dev,
					"%s(): SPI Write failed rc = %d\n",
					__func__, rc);
				...
				current_vaddr += ES_SPI_DMA_MIN_BYTES;
				non_dma_bytes -= ES_SPI_DMA_MIN_BYTES; 
			}
			kfree(kaddr);
		}
...
static int escore_spi_populate_dt_data(struct escore_priv *escore)
{
	...
	pr_info("%s, parsing adnc config is success\n",
				__func__);
...
static const struct spi_device_id escore_spi_id[] = {
	{ "earSmart-codec-spi", 0},
	{ }
};
MODULE_DEVICE_TABLE(spi, escore_id);

#ifdef CONFIG_OF
static const struct of_device_id es814_spi_of_ids[] = {
    {.compatible = "mediatek,es814_spi",},
    {}
};
#endif
...
struct spi_driver escore_spi_driver = {
	.driver = {
		.name   = "earSmart-codec-spi",
		.bus    = &spi_bus_type,
		.owner  = THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = es814_spi_of_ids,
#endif
		.pm = &escore_pm_ops,
	},
