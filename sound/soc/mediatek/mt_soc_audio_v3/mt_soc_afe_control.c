...
//modify for phone mic to headset loopback
bool AudioAdcI2SStatus = false;
#else
bool AudioAdcI2SStatus = false;
#endif
...
bool SetI2SAdcIn(AudioDigtalI2S *DigtalI2S)
{
	...
	uint32 eSamplingRate = 0;
	uint32 dVoiceModeSelect = 0;

	memcpy((void *)AudioAdcI2S, (void *)DigtalI2S, sizeof(AudioDigtalI2S));

	//set reg for Internal ADC
	eSamplingRate = SampleRateTransform(AudioAdcI2S->mI2S_SAMPLERATE);
	...
	pr_info("%s Audio_I2S_Adc = 0x%x, AudioAdcI2SStatus is %d\n", __func__, Audio_I2S_Adc, AudioAdcI2SStatus);
	Afe_Set_Reg(AFE_I2S_CON2, Audio_I2S_Adc, MASK_ALL);

	return true;
}

bool SetI2SAdcSwitch(bool enable)
{
	if (false == enable) {
...
uint32 GetI2SAdcSwitch(void)
{
	uint32 adcStatus = 0;
	adcStatus = Afe_Get_Reg(AFE_ADDA_TOP_CON0) & 0x1;
	return adcStatus;
}
#else
bool SetI2SAdcIn(AudioDigtalI2S *DigtalI2S)
{
	...
	pr_info("%s AudioAdcI2SStatus = %d\n", __func__, AudioAdcI2SStatus);
...
bool SetI2SDacOut(uint32 SampleRate, bool lowjitter, bool I2SWLen)
{
	...
	pr_warn("SetI2SDacOut Audio_I2S_Dac %x\n", Audio_I2S_Dac);
