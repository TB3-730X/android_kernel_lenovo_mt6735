...
enum audio_system_gpio_type {
	...
	GPIO_RCVSPK_HIGH,
	GPIO_RCVSPK_LOW,
	GPIO_HPANASWITCH_HIGH,
	GPIO_HPANASWITCH_LOW,
	GPIO_NUM
};
...
static struct audio_gpio_attr aud_gpios[GPIO_NUM] = {
	...
	[GPIO_HPANASWITCH_HIGH] = {"hpanaswitch-pullhigh", false, NULL},
	[GPIO_HPANASWITCH_LOW] = {"hpanaswitch-pulllow", false, NULL},
};
...
#define GPIO_SPK_PA_EN         (GPIO4 | 0x80000000)
#define GPIO_SPK_PA_EN_M_GPIO   GPIO_MODE_00
int AudDrv_GPIO_EXTAMP_Select_Mode(int mode, bool enable)
{
	int retval = 0, i = 0;

	mt_set_gpio_mode(GPIO_SPK_PA_EN, GPIO_SPK_PA_EN_M_GPIO);
	mt_set_gpio_dir(GPIO_SPK_PA_EN, GPIO_DIR_OUT);

	if (enable) {
		for(i = 0; i < mode; i++) {
			mt_set_gpio_out(GPIO_SPK_PA_EN, 0);
			mt_set_gpio_out(GPIO_SPK_PA_EN, 1);
		}
	} else {
		mt_set_gpio_out(GPIO_SPK_PA_EN, 0);
	}
...
int AudDrv_GPIO_HP_ANA_Select(int bEnable)
{
	int retval = 0;

	if (bEnable == 1) {
		if (aud_gpios[GPIO_HPANASWITCH_HIGH].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_HPANASWITCH_HIGH].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_HPANASWITCH_HIGH] pins\n");
		}
	} else {
		if (aud_gpios[GPIO_HPANASWITCH_LOW].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_HPANASWITCH_LOW].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_HPANASWITCH_LOW] pins\n");
		}
