...
extern int es705_remote_add_codec_controls(struct snd_soc_codec *codec);
...
#define AW8738_MODE 3

extern int AudDrv_GPIO_EXTAMP_Select_Mode(int mode, bool enable);
...
#define SPK_WARM_UP_TIME        (25)	/* unit is ms */
	...
	if (enable) {
		...
		//AudDrv_GPIO_EXTAMP2_Select(false);
		mutex_lock(&Ana_Ctrl_Mutex);
		AudDrv_GPIO_EXTAMP_Select_Mode(3, 1);
		mutex_unlock(&Ana_Ctrl_Mutex);
#endif /*CONFIG_MTK_LEGACY*/
...
#else
		AudDrv_GPIO_EXTAMP_Select_Mode(3, 0);
		udelay(1*1000);
...
static int Audio_Hp_Ana_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s()\n", __func__);
	ucontrol->value.integer.value[0] =
	    mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HP_ANA_SWITCH];
	return 0;
}

static int Audio_Hp_Ana_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{

	pr_debug("%s() gain = %ld\n ", __func__, ucontrol->value.integer.value[0]);
	if (ucontrol->value.integer.value[0]) {
		AudDrv_GPIO_HP_ANA_Select(true);
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HP_ANA_SWITCH] =
		    ucontrol->value.integer.value[0];
	} else {
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_HP_ANA_SWITCH] =
		    ucontrol->value.integer.value[0];
		AudDrv_GPIO_HP_ANA_Select(false);
	}
	return 0;
}

static int Audio_Rec_Amp_Get(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s()\n", __func__);
	ucontrol->value.integer.value[0] =
	    mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_REC_AMP_SWITCH];
	return 0;
}

static int Audio_Rec_Amp_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s() gain = %ld\n ", __func__, ucontrol->value.integer.value[0]);
	if (ucontrol->value.integer.value[0]) {
		mutex_lock(&Ana_Ctrl_Mutex);
		AudDrv_GPIO_EXTAMP_Select_Mode(5, 1);
		mutex_unlock(&Ana_Ctrl_Mutex);
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_REC_AMP_SWITCH] =
		    ucontrol->value.integer.value[0];
	} else {
		AudDrv_GPIO_EXTAMP_Select_Mode(5, 0);
		udelay(500);
		mCodec_data->mAudio_Ana_DevicePower[AUDIO_ANALOG_DEVICE_OUT_REC_AMP_SWITCH] =
		    ucontrol->value.integer.value[0];
	}
...
static const struct snd_kcontrol_new mt6331_snd_controls[] = {
	...
	SOC_ENUM_EXT("Ext_Speaker_Amp_Switch", Audio_DL_Enum[11], Ext_Speaker_Amp_Get,
		     Ext_Speaker_Amp_Set),
#ifdef CONFIG_OF
	SOC_ENUM_EXT("Audio_HP_Ana_Switch", Audio_DL_Enum[11], Audio_Hp_Ana_Get, Audio_Hp_Ana_Set),
	SOC_ENUM_EXT("Audio_REC_Amp_Switch", Audio_DL_Enum[11], Audio_Rec_Amp_Get, Audio_Rec_Amp_Set),
#endif
...
static bool GetAdcStatus(void)
{
	...
	for (i = AUDIO_ANALOG_DEVICE_IN_ADC1; i < AUDIO_ANALOG_DEVICE_MAX; i++) {
		if ((mCodec_data->mAudio_Ana_DevicePower[i] == true)
		    ...
		    && (i != AUDIO_ANALOG_DEVICE_OUT_HP_ANA_SWITCH) && (i != AUDIO_ANALOG_DEVICE_OUT_REC_AMP_SWITCH)) {
			return true;
		}
	}
	return false;
